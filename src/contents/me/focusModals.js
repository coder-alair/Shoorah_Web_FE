...
const FocusModal = ({
  ...
  const handleClick = (i) => {
    if (myFocus.includes(i.focusId)) {
      let newArray = myFocus.filter((item) => item != i.focusId);
      setMyfocus(newArray);
    } else {
      setMyfocus([...myFocus, i.focusId]);
    }
  };
        ...
        <div className="fixed inset-0 z-20 overflow-y-auto">
          <div className="flex min-h-full items-end  justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <Dialog.Panel
                ref={cancelButtonRef}
                className="relative h-[85vh] w-full transform overflow-hidden rounded-3xl bg-white px-5 text-left shadow-xl transition-all sm:my-8 lg:h-auto xl:w-[80vw]  "
              >
                <div className="h-full lg:h-auto">
                  ...
                  <div className="relative flex h-full flex-col rounded-3xl py-2 lg:h-fit xl:px-5 ">
                    <div className="relative flex justify-between py-3 xl:px-5">
                      <p className="P22Mackinac cursor-default text-lg lg:text-2xl">
                        Select Your Focuses
                      </p>
                      <p
                        onClick={() => {
                    ...
                    <div
                      className={`${
                        audioNav && `mb-[4rem]`
                      } mt-4 flex justify-between px-5 py-3 xl:mt-[3rem]`}
                    >
                      <button
                        onClick={handleSave}
