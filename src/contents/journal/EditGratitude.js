...
const pages = [
  ...
  { name: "Edit Gratitude", href: "/journal/edit-gratitude", current: true },
];

export default function EditGratitude() {
  const location = useLocation();
  let gratitude = location.state;
  const [text, setText] = useState(gratitude.description);
  const [title, setTitle] = useState(gratitude.displayName);
  const [preview, setPreview] = useState(gratitude.imageUrl);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      const payload = {
        userGratitudeId: gratitude.userGratitudeId,
        title: title,
      ...
      Api.addGratitude(payload).then((res) => {
        if (res.status == 200) {
            ...
            axios
              ...
              .then((res) => {
                if (res.status == 200) {
                  ...
                  setText("");
                  setTitle("");
                  successToast("Gratitude is updated successfully !");
                  history.replace("/journal/gratitude");
                }
              })
              .catch((err) => {
                setLoader(false);
  ...
  return (
    <div className={`${audioNav && `mb-[2rem]`} pb-[13vh] lg:pb-0`}>
      <Navbar />
      <Hero hideInfo={true} heading={`Edit Gratitude`} />
      <SideMenu />
      {loader && <Loader />}
              ...
              <div className="flex flex-col justify-around gap-3">
                ...
                <div className="w-[100%]">
                  <div className="relative flex flex-col gap-2">
                    <label htmlFor="description" className="ml-5 mt-2 text-xl">
                      Description
                    </label>
                ...
                <div className=" mt-8 flex w-full justify-around gap-[1rem] text-center sm:gap-[3rem] lg:gap-[8rem]">
                  <button
                    onClick={handleDraft}
                    className="P22Mackinac w-[50%] rounded-[3rem] border border-[gray] border-transparent bg-[#fff] py-3 text-base font-medium text-[#000] shadow-sm hover:bg-[gray] hover:text-white focus:outline-none focus:ring-2 focus:ring-[gray] focus:ring-offset-2 sm:py-3 xl:text-xl"
                  >
                    Draft
                  </button>
                  <button
                    type="submit"
                    name="submit"
                    className={`w-[50%] border border-transparent ${theme.shoorah_bg_5} ${theme.shoorah_bg_hover_6} P22Mackinac rounded-[3rem] py-2 text-base font-medium text-white shadow-sm focus:outline-none focus:ring-2 sm:py-3 xl:text-xl ${theme.shoorah_border_ring_focus_5} focus:ring-offset-2`}
                  >
                    Save
                  </button>
                </div>
