...
export default function Cleanse() {
  ...
  const [cleanses, setCleanses] = useState([]);
  ...
  const getCleanse = () => {
    ...
    Api.getCleanseList(true, 1, 50, searchKey)
      .then((res) => {
        if (res?.data?.meta?.code === 9) {
          setLoader(false);
          errorToast(res.data.meta.message);
        } else {
          setCleanses(res.data.data);
  ...
  const getDrafts = () => {
    ...
    Api.getCleanseList(false, 1, 50, searchKey)
      .then((res) => {
        if (res?.data?.meta?.code === 9) {
          setLoader(false);
          errorToast(res.data.meta.message);
  ...
  const handleDelete = (id) => {
    Api.deleteCleanse(id)
      .then((res) => {
        if (res?.data?.meta?.code === 9) {
          setLoader(false);
          errorToast(res.data.meta.message);
        } else {
          successToast("Cleanse Deleted Successfully");
    ...
    Api.getCurrentPlan().then((res) => {
      if (res.data.meta.code == 1) {
        if (res?.data?.data?.accountType != "SUBSCRIBED") {
          if (cleanses.length >= 1) {
            history.replace("/subscription");
          } else {
            history.push("/journal/add-cleanse");
          }
        }
        else {
          history.push('/journal/add-cleanse');
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden pb-[13vh] lg:pb-0">
      <Navbar />
      <Hero hideInfo={true} heading={`My Cleanse`} />
      <SideMenu />
      {loader && <Loader />}
      <div className=" mx-auto w-full xl:mt-10 xl:w-[70vw]">
        <div className="w-full">
          <div className="mb-1 mt-4 flex items-center justify-around px-4 py-3 lg:mt-10 xl:px-10">
            <div className="relative flex w-full justify-center xl:w-[90%] xl:pl-4">
              <BackButton />

              <SearchInputField
                placeholder={"Search Cleanse here"}
                searchKey={searchKey}
            ...
            <div onClick={handleClick} className="cursor-pointer rounded-full ">
              <div
                className={` aspect-square w-12 rounded-[100%] xl:w-[4rem] ${theme.shoorah_bg_5} ${theme.shoorah_bg_hover_4} ml-2 xl:ml-[3rem] xl:h-[4rem]`}
              >
          ...
          <div className="mx-auto w-full">
            <div className="scrollbar px-4 py-3 xl:px-10">
              {drafts.length ? (
                <p className="P22Mackinac mb-1 mt-4 text-xl lg:mt-8 xl:text-3xl">
                  Drafts
                </p>
              ) : null}
              {drafts.map((item) => (
                <div key={item.cleanseId}>
                  <ImageCard
                    title={item.title}
                    url={"/journal/edit-cleanse"}
                    data={item}
                    image={item.imageUrl}
                    id={item.cleanseId}
          ...
          <div className="scrollbar px-4 py-5 xl:px-10">
            {cleanses.length ? (
              <p className="P22Mackinac text-xl xl:my-10 xl:text-3xl">
                My Cleanse
              </p>
            ) : null}
            {cleanses.map((item) => (
          ...
          {!filterData.length && !filterDrafts.length && (
            <div className="">
              <p className="P22Mackinac ml-5 text-2xl">No Cleanse</p>
            </div>
          )}
        </div>
      </div>
