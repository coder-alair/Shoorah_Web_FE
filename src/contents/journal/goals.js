...
import image from "../../assets/images/reset-password-screen-image.png";
import Hero from "../reusable/hero";
import Navbar from "../layout/navbar";
import SideMenu from "../layout/sideMenu";
import Loader from "../../component/common/Loader";
import ImageCard from "../reusable/imageCard";
import GoalsAcheived from "./goalsAcheived";
import GoalsIncomplete from "./goalsIncomplete";
...
const pages = [
  ...
  { name: "Cleanse", href: "/journal/cleanse", current: true },
];
...
export default function Goals() {
  ...
  // }, [searchKey, drafts, goals, refresh]);
  ...
  const getGoalsList = () => {
    ...
    Api.getGoals(1, 50, true, searchKey)
      .then((res) => {
  ...
  const getProgressGoalsList = () => {
    ...
    Api.getGoals(1, 50, false, searchKey)
      .then((res) => {
        if (res?.data?.meta?.code === 9) {
          setLoader(false);
          errorToast(res.data.meta.message);
          ...
          setProgressGoals(sorting);
          setFilterProgress(sorting);
          setLoader(false);
        }
      })
      .catch((err) => {
    ...
    Api.getCurrentPlan().then((res) => {
      if (res.data.meta.code == 1) {
        if (res?.data?.data?.accountType != "SUBSCRIBED") {
          if (goals.length + progressGoals.length >= 1) {
            history.replace("/subscription");
          } else {
            history.push("/journal/add-goals");
          }
        } else {
          history.push("/journal/add-goals");
        }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden pb-[13vh] lg:pb-0">
      <Navbar />
      <Hero hideInfo={true} heading={`My Goals`} />
      <SideMenu />
      {loader && <Loader />}
      <div className=" mx-auto mt-4 w-full lg:mt-10 xl:w-[70vw]">
        <div className="w-full">
          <div className="flex items-center justify-between py-3 xl:px-10">
            <div className="relative flex w-[90%] justify-center pl-4">
              <BackButton />

              <SearchInputField
                placeholder={"Search Goals here"}
                searchKey={searchKey}
            ...
            <div onClick={handleClick} className="cursor-pointer rounded-full ">
              <div
                className={` h-12 w-12 rounded-[100%] xl:h-[4rem] xl:w-[4rem] ${theme.shoorah_bg_5} ${theme.shoorah_bg_hover_4} mx-4 xl:ml-[3rem] `}
              >
                <button className="flex h-[100%] w-[100%] items-center justify-center text-[#fff]">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
          ...
          <div className="mx-auto w-full ">
            <div className="scrollbar px-4 py-3 xl:px-10">
              {drafts.length ? (
          ...
          <div className="mb-[1rem] flex items-center justify-center px-4 sm:mt-8 lg:mt-[4rem] xl:gap-[5rem] xl:px-0">
            <div
              onClick={() => setTab(0)}
              className={
                tab == 0
                  ? `rounded-[3rem] text-lg font-medium lg:text-xl ${theme.shoorah_bg_1}  flex  items-center justify-center px-8 py-1 text-center  md:px-12 lg:px-16 lg:py-3 ${theme.shoorah_text_5} P22Mackinac ease cursor-pointer duration-1000`
                  : `rounded-[3rem] text-lg font-medium lg:text-xl ${theme.inactiveToggleText} P22Mackinac ease flex  cursor-pointer  items-center justify-center px-8 py-1 text-center duration-1000 md:px-12 lg:px-16 lg:py-3`
              }
            >
              In Progress
            </div>
