...
const pages = [
  ...
  { name: "Notepad", href: "/journal/notepad", current: true },
];
...
export default function Notepad() {
  ...
  // }, [searchKey, drafts, notes, refresh]);
  ...
  const getNotes = () => {
    ...
    Api.getNotesList(true, 1, 50, searchKey)
      .then((res) => {
  ...
  const getDrafts = () => {
    ...
    Api.getNotesList(false, 1, 50, searchKey)
      .then((res) => {
        if (res?.data?.meta?.code === 9) {
          setLoader(false);
          errorToast(res.data.meta.message);
      ...
      if (res.data.meta.code == 1) {
        if (res?.data?.data?.accountType != "SUBSCRIBED") {
          if (notes.length >= 1) {
            ...
            history.push("/journal/add-notes");
          }
        }
        else {
          history.push('/journal/add-notes');
  ...
  return (
    <div className="relative min-h-screen overflow-hidden pb-[13vh] lg:pb-0">
      <Navbar />
      <Hero hideInfo={true} heading={`My Notepad`} />
      <SideMenu />
      {loader && <Loader />}
      <div className=" mx-auto mt-8 w-full lg:mt-10 xl:w-[70vw]">
        <div className="w-full">
          <div className="flex items-center justify-around gap-3 px-4 xl:mt-10">
            ...
            <div onClick={handleClick} className="cursor-pointer rounded-full ">
              <div
                className={` h-12 w-12 rounded-[100%] xl:w-[4rem] ${theme.shoorah_bg_5} ${theme.shoorah_bg_hover_4} ml-2 xl:ml-[3rem] xl:h-[4rem]`}
              >
          ...
          <div className="mx-auto w-full px-4 xl:my-6 xl:w-[70vw] xl:px-0">
            <div className="scrollbar px-1 py-3">
              {drafts.length ? (
              ...
              {drafts.map((item) => (
                <div key={item.notesId}>
                  <ImageCard
                    note={true}
                    title={item.title}
                    url={"/journal/edit-note"}
                    data={item}
                    image={item.imageUrl}
                    id={item.notesId}
          ...
          <div className="scrollbar mb-[1.2rem]  px-4 py-3 xl:px-1">
            {notes.length ? (
              <p className="P22Mackinac mt-4 text-xl lg:mt-8 xl:text-3xl">
                My Notes
              </p>
            ) : null}
            {notes.map((item) => (
          ...
          {!filterData.length && !filterDrafts.length && (
            <div className="mb-10  ">
              <p className="P22Mackinac ml-5 text-2xl">No Notes</p>
            </div>
          )}
        </div>
      </div>
