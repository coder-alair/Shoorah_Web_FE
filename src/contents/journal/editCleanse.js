...
import CleanseModal from "../reusable/cleanseModal";
...
export default function EditCleanse() {
  const location = useLocation();
  const { theme } = useTheme();
  let cleanse = location.state;
  const [text, setText] = useState(cleanse.description);
  const [title, setTitle] = useState(cleanse.title);
  const [preview, setPreview] = useState(cleanse.imageUrl);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      Api.addCleanse(payload).then((res) => {
        if (res.status == 200) {
            ...
            axios
              ...
              .then((res) => {
                if (res.status == 200) {
                  ...
                  setText("");
                  setTitle("");
                  successToast("Your mind is now cleansed of these thoughts!");
  ...
  return (
    <div className={`${audioNav && `mb-[2rem]`} pb-[13vh] lg:pb-0`}>
      <Navbar />
      <Hero hideInfo={true} heading={`Edit Cleanse`} />
      <SideMenu />
      {loader && <Loader />}
              ...
              <div className="flex flex-col justify-around gap-3">
                ...
                <div className=" mt-8 flex w-full justify-around gap-[1rem] text-center sm:gap-[3rem] lg:gap-[8rem]">
                  ...
                  <button
                    type="submit"
                    name="submit"
                    className={`w-[50%] border border-transparent ${theme.shoorah_bg_5} ${theme.shoorah_bg_hover_6} P22Mackinac rounded-[3rem] py-2 text-base font-medium text-white shadow-sm sm:py-3 xl:text-xl ${theme.bgPrimaryHover} focus:outline-none focus:ring-2 ${theme.shoorah_border_ring_focus_5} focus:ring-offset-2`}
                  >
                    Save
                  </button>
                </div>
      ...
      <CleanseModal open={show} setOpen={setShow} />
    </div>
  );
}
