...
const MeditationBookmarks = () => {
  ...
  const getBookmarks = () => {
    setLoader(true);
    Api.getMyBookmarks(BOOKMARKS_CONTENT_TYPE.MEDITATION)
      .then((res) => {
        if (res.data.meta.code == 1) {
          setMeditation(res.data.data);
          setLoader(false);
        } else {
  ...
  return (
    <div className="mx-auto items-center justify-evenly md:justify-start lg:my-3 lg:mt-[2rem] flex w-[95%] md:w-[86%] lg:w-[76%] flex-wrap gap-x-5 px-4">
      {loader && <Loader />}
      {meditation.map((i) => (
        <div
          onClick={() =>
      ...
      {!meditation.length && (
        <div className="P22Mackinac mt-[5rem] flex justify-center items-center lg:mt-[3rem] h-full lg:h-[10rem] text-md md:text-lg lg:text-2xl">
          No Meditation
        </div>
      )}
    </div>
...
export default MeditationBookmarks;
