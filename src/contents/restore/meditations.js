...
const MeditationsPage = () => {
  ...
  const [secondMeditations, setSecondMeditations] = useState([]);
  ...
  const apiCall = async () => {
    ...
    await getSecondMeditationSounds();
    setIsLoading(false);
  };
  ...
  const getMeditationSounds = async () => {
    ...
    Api.getMeditationSounds(1, 20, searchKey, category._id)
      .then((res) => {
        if (res.data.meta.code == 1) {
          setLoader(false);
  ...
  const getSecondMeditationSounds = async () => {
    setLoader(true);
    let search = searchKey.toLowerCase();
    if (
      search.startsWith("#") ||
    ...
    Api.getMeditationSounds(2, 20, searchKey, category._id)
      .then((res) => {
        if (res.data.meta.code == 1) {
          setLoader(false);
          setSecondMeditations(res.data.data);
          // setPage(prevPage => prevPage + 1);
        } else {
          setLoader(false);
          errorToast(res.data.meta.message);
  ...
  return (
    <div className="relative mb-[1rem] overflow-hidden px-4 md:mb-[5rem] xl:px-0">
      {loader && <Loader />}
      ...
      <div
        style={{ rotate: "20deg" }}
        className="absolute bottom-[5%] left-[-15%] z-[-3]  w-[14rem] md:w-[20rem] lg:w-[28rem]"
      >
        <svg
          // width={1440}
          // height={1440}
      ...
      <div className="mx-auto mt-8 flex w-full flex-col justify-around gap-4 sm:mt-4 xl:w-[76%] xl:flex-row xl:gap-10  ">
        <CategoryFilter
          apiCall={apiCall}
        ...
        />
        <SearchInputField
          placeholder={`Search meditations here`}
          searchKey={searchKey}
      ...
      {isLoading ? (
        ...
        <>
          <div className=" mx-auto flex w-[95%]  items-center justify-center md:mt-[2rem]">
            {meditations.length > 0 && (
              <NewCarousel data={meditations} left={true} />
            )}
            {meditations.length == 0 && !isLoading && (
              <p className="text-2xl">No Meditation</p>
            )}
          </div>

          <div className="mx-auto mt-2 flex w-[95%]  items-center justify-center md:mt-[2rem]">
            {secondMeditations.length > 0 && (
              <NewCarousel data={secondMeditations} left={true} />
            )}
          </div>
        </>
      )}
